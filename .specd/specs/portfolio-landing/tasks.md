...
- [ ] T1 — Create LandingController and route
  ...
  - verify: "php artisan route:list | grep '^GET /'  && curl -s http://localhost:8000/ | grep -q '<html'"
  - depends: —
  - requirements: 1, 2, 3, 4, 5, 6

- [ ] T2 — Create landing.blade.php view skeleton
  ...
  - verify: "grep -c '<section\\|<article' resources/views/landing.blade.php | grep -q '[1-6]' && grep -c 'var(--' resources/css/design-system.css | awk '{print ($1 > 50) ? \"OK\" : \"FAIL\"}'"
  - depends: T1
  - requirements: 1, 2, 8

- [ ] T3 — Implement Hero section (full-viewport, black bg, NDot headline, CTA)
  ...
  - verify: "curl -s http://localhost:8000/ | grep -q 'height.*100vh\\|clamp(48px' && npm run build && php artisan serve &; sleep 2; screenshot localhost:8000 | grep -q 'black background\\|white headline'"
  - depends: T2
  - requirements: 1

## Wave 2

- [ ] T4 — Implement Navigation bar (fixed, scroll detection, adaptive color)
  ...
  - verify: "grep -A5 'nav.*fixed' resources/css/design-system.css && grep -q 'scroll.*addEventListener\\|backdrop-blur' resources/js/nav.js && curl -s http://localhost:8000/ | grep -q '<nav class=.*fixed'"
  - depends: T3
  - requirements: 2

- [ ] T5 — Implement Featured projects grid (responsive, 2→1 columns, hover scale)
  ...
  - verify: "curl -s http://localhost:8000/ | grep -c '<img.*aspect-ratio' | awk '{print ($1 >= 2) ? \"OK\" : \"FAIL\"}' && npm run build && screenshot localhost:8000 --viewport 375x667 | grep -q 'single column'"
  - depends: T2
  - requirements: 3

- [ ] T6 — Implement Recent articles grid (responsive, 3→1 columns, black bg, red "View All" link)
  ...
  - verify: "curl -s http://localhost:8000/ | grep -A2 'Recent\\|Articles' | grep -q 'var(--color-pure-black)' && screenshot localhost:8000 | grep -q 'red link'"
  - depends: T2
  - requirements: 4

- [ ] T7 — Implement Packages preview grid (3–4 feature cards with icons, "Explore All" button)
  ...
  - verify: "curl -s http://localhost:8000/ | grep -c 'Explore All' | awk '{print ($1 == 1) ? \"OK\" : \"FAIL\"}' && grep -q 'icon.*48px' resources/css/design-system.css"
  - depends: T2
  - requirements: 5

- [ ] T8 — Implement CTA sections and footer
  ...
  - verify: "curl -s http://localhost:8000/ | grep -c 'section.*cta\\|footer' | awk '{print ($1 >= 2) ? \"OK\" : \"FAIL\"}' && grep -q 'social.*24px' resources/css/design-system.css"
  - depends: T2
  - requirements: 6, 7

## Wave 3

- [ ] T9 — Implement responsive design (mobile-first, hamburger menu on <1024px)
  ...
  - verify: "screenshot localhost:8000 --viewport 375x667 | grep -q 'hamburger' && screenshot localhost:8000 --viewport 1440x900 | grep -q -v 'hamburger' && npm run build && npm run test:responsive"
  - depends: T4, T5, T6, T7
  - requirements: 9

- [ ] T10 — Verify accessibility (WCAG AA, semantic HTML, keyboard navigation, screen reader)
  ...
  - verify: "npm run audit:a11y && npm run test:lighthouse | grep -q 'Accessibility.*9[5-9]' && npm run test:keyboard-nav"
  - depends: T8
  - requirements: 10

- [ ] T11 — Verify responsive design breakpoints and performance (Lighthouse >85, CLS <0.1, FCP <2s)
  ...
  - verify: "npm run build && npm run test:lighthouse | grep -E 'Performance.*8[5-9]|9[0-9]|100' && npm run test:performance --mobile | grep -q 'CLS.*0\\.[0-9]' && npm run test:responsive"
  - depends: T9, T10
  - requirements: 9, 10

- [ ] T12 — Final integration test: render full landing page, verify all sections, all tokens, no errors
  ...
  - verify: "npm run build && npm run lint && curl -s http://localhost:8000/ | grep -q 'var(--' | wc -l | awk '{print ($1 > 50) ? \"OK\" : \"FAIL\"}' && npm run test:lighthouse | grep -q 'Performance.*8[5-9]'"
  - depends: T11
